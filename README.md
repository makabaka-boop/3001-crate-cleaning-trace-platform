...
- 库位盘点：从盘点页选择库位创建独立盘点单，以创建时的在用箱体快照作为核对基准；逐个扫描主编号或备用编号后提交，页面展示应在、实扫、缺失和错放明细，已完成盘点保留供当次结果复看。完成时统一解析编号、拒绝重复扫描，快照与差异结果在同一事务内固化，之后的箱体位置变更不改写历史结论；空库位、未知编号、已停用箱和重复完成分别返回可定位的业务反馈，提交失败时页面保留未提交的扫描内容，可修正后重试。
...
- `POST /api/inventory-checks`（按库位创建盘点单，以创建时的在用箱体快照为核对基准；空库位返回 404）；`GET /api/inventory-checks`、`GET /api/inventory-checks/{id}`（盘点记录与详情，已完成盘点保留应在/实扫/缺失/错放结果供复看）；`POST /api/inventory-checks/{id}/complete`（完成盘点：统一解析主/备编号，未知编号 404、已停用箱与重复扫描 409 且盘点单保持进行中可修正重试，重复完成 409；快照与差异结果同一事务固化，之后的箱体变更不改写历史）
